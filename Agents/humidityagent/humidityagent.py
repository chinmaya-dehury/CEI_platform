...
def register_with_consul():
    try:
        service = {
            "ID": metadata["uuid"],
            "Name": metadata["agent_name"],  # Always use agent name dynamically
            "Address": metadata["address"],  #  Use controller-assigned address
            "Port": PORT,
            "Meta": {
            ...
            "Check": {
                "HTTP": f"http://{metadata['address']}:{5003}/health",  # Dynamic health check URL
                "Interval": "10s"
            }
        }
